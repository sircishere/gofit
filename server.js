...
app.post("/logWorkout",requiresAuth(), async (req,res) => {
  ...
  try{
    ...
    for(let i = 0; i < req.body.sets.length; i++){
      const query = "INSERT INTO workout_log (user_id,workout_name,weight,rep) VALUES (?,?,?,?)"
      const [response] = await db.query(query, [id[0].id,req.body.workoutName, req.body.sets[i].weight, req.body.sets[i].rep]);
    }
